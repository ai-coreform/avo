...
/**
 * Same as getFullMenu but:
 * - Accepts venueId (from auth) instead of venueSlug
 * - Works for any menu status (draft, published, archived)
 * - Includes all items regardless of visibility
 */
export async function getPreviewMenu(venueId: string, menuSlug: string) {
  ...
  // No status filter — preview works for any status
  const [menuRow] = await database
    .select()
    .from(menu)
    .where(and(eq(menu.venueId, venueId), eq(menu.slug, menuSlug)))
    .limit(1);
  ...
  // No isVisible filter — preview shows everything
  const tabRows = await database
    .select()
    .from(menuTab)
    .where(eq(menuTab.menuId, menuRow.id))
    .orderBy(asc(menuTab.sortOrder), asc(menuTab.createdAt));

  const categoryRows = await database
    .select()
    .from(menuCategory)
    .where(eq(menuCategory.menuId, menuRow.id))
  ...
  const entryRows =
    categoryIds.length > 0
      ? await database
          ...
          .where(eq(menuEntry.menuId, menuRow.id))
  ...
  const promotionRows = await database
    .select()
    .from(promotion)
    .where(eq(promotion.menuId, menuRow.id))
  ...
  const scheduleRows =
    promotionIds.length > 0
      ? await database
          ...
          .where(inArray(promotionSchedule.promotionId, promotionIds))
      : [];
