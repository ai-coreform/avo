...
function AvoAvatar({
  ...
  return (
    ...
    >
      ...
      <img alt="" className="size-full object-contain" src="/images/avo-icon-white.svg" />
    </div>
  );
}
...
export function AvoChatPanel({
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-center bg-black/20">
      ...
      {/* Chat body */}
      <div className="flex-1 overflow-y-auto [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" ref={scrollRef}>
       <div className="mx-auto w-full max-w-2xl px-4 py-3">
        {showWelcome && (
          <div className="flex flex-col items-center px-2 pt-4 pb-2">
            <AvoMascot bgColor={bgColor} size={100} />
            <h2 className="mt-4 font-bold text-xl text-[#333]">
              {strings.welcomeTitle}
            </h2>
            <p className="mt-1 text-center text-[13px] text-[#aaa]">
              {strings.welcomeSubtitle}
            </p>
            <div className="mt-4 flex flex-col items-center gap-2">
              {suggestions.map((suggestion) => (
                <button
                  className="rounded-[20px] border border-[#ddd] px-5 py-2 text-center text-[13px] text-[#333] transition-colors hover:bg-[#f5f5f5]"
                  key={suggestion}
          ...
          if (message.role === "assistant") {
            ...
            return (
              <div className="mb-3 flex items-start gap-2" key={message.id}>
                <AvoAvatar bgColor={bgColor} size={28} />
                <div className="max-w-[80%] rounded-2xl rounded-tl-sm bg-[#f0f0f0] px-3.5 py-2 text-[13px] text-[#333] leading-relaxed whitespace-pre-wrap">
                  {renderMarkdown(text)}
                </div>
              </div>
            );
        ...
        {errorMessage && (
          <div className="mb-3 flex items-start gap-2.5 rounded-xl border border-[#f4d4d4] bg-[#fef5f5] px-3.5 py-2.5">
            <p className="flex-1 text-[12px] text-[#8a4040] leading-relaxed">
              {errorMessage}
            </p>
            <button
              className="flex shrink-0 items-center gap-1 rounded-md bg-white px-2.5 py-1 font-medium text-[11px] text-[#8a4040] shadow-sm transition-colors hover:bg-[#fafafa]"
              onClick={() => {
                clearError();
                regenerate().catch(() => {});
              }}
        ...
        {followUpSuggestions.length > 0 && (
          <div className="mt-1 mb-2 flex flex-wrap gap-1.5 pl-[36px]">
            {followUpSuggestions.map((s) => (
              <button
                className="rounded-[20px] border border-[#ddd] px-3 py-1.5 text-[12px] text-[#666] transition-colors hover:bg-[#f5f5f5]"
                key={s}
                onClick={() => fireSend(s)}
                type="button"
              >
      ...
      >
       <div className="mx-auto w-full max-w-2xl px-4 pt-2">
        ...
        >
          ...
          >
            <textarea
              className="min-h-[44px] max-h-[120px] flex-1 resize-none bg-transparent px-3 py-2 font-sans text-base outline-none"
              disabled={isBusy}
